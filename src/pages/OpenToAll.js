...
const OpenToAll = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const apiUrl = `${process.env.REACT_APP_SERVER}/opentoallattendees/${data}`;
  ...
  const newData = `${process.env.REACT_APP_FRONT}/opentoallform/${data}`;
  ...
  return (
    <div className="w-full h-screen overflow-y-scroll ">
      <div className="px-2 md:px-8 text-[14px] md:text-[16px] ">
        ...
        <p>Key: </p>
        <div>
          <CopyLink link={newData} />
        </div>
        <div>
          <SocialMedia link={newData} />
        </div>
        <div>
